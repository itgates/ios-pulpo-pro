...
//    @IBOutlet weak var heightTableView: NSLayoutConstraint!
    ...
    private var selectedFilterType: FilterType?
    private var tableObservation: NSKeyValueObservation?
    private var sections: [VisitSection] = []
...
private extension ActualVisitVC {
    ...
    func styleDateView(_ view: UIView) {
        ...
        style(
            view: view,
            cornerRadius: view.frame.height / 2,
            borderWidth: 2,
...
private extension ActualVisitVC {
    ...
    func configureTableView() {
        ...
        tableView.register(UINib(nibName: "CellOfficeWork", bundle: nil), forCellReuseIdentifier: "CellOfficeWork")
...
extension ActualVisitVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(
        ...
        if let model = item as? ActualVisitModel {
            ...
            let cell = tableView.dequeueReusableCell(
                withIdentifier: "CellActualVisit",
                for: indexPath
            ) as! CellActualVisit

            cell.viewContiner.layer.cornerRadius = 8
            shadowView(cell.viewContiner, color: .gray, opacity: 0.13, offset: .zero, radius: 10)

            cell.configure(with: model)
            cell.onMapTapped = { [weak self] in
                if model.lgAcccount.isEmpty || model.llAcccount.isEmpty {
        ...
        if let model = item as? OWSModel {

            let cell = tableView.dequeueReusableCell(
                withIdentifier: "CellOfficeWork",
                for: indexPath
            ) as! CellOfficeWork
            cell.viewContiner.layer.cornerRadius = 8
            shadowView(cell.viewContiner, color: .gray, opacity: 0.13, offset: .zero, radius: 10)

            let owViewModel = OWActivitiesViewModel()
            owViewModel.fetchData()
            cell.configure(with: model, viewModel: owViewModel)
...
private extension ActualVisitVC {
    ...
    func bindActions() {
        ...
        stackColapseTapped.onTap { [weak self] in
            self?.toggleFilterStack()
        }

        viewTappedDateFrom.onTap { [weak self] in
            self?.openCalendar(for: .dateFrom)
        }

        viewTappedDateTo.onTap { [weak self] in
            self?.openCalendar(for: .dateTo)
        }

        applyButton.rx.tap
            .bind { [weak self] in self?.applyDateFilter() }
            .disposed(by: disposeBag)
    }

    func applyDateFilter() {

        guard let fromText = dateFromLabel.text,
              let toText = dateToLabel.text else { return }
        ...
        viewModel.filterVisits(from: fromDate, to: toDate)
...
private extension ActualVisitVC {
    ...
    func toggleFilterStack() {
        stackFilter.isHidden ? showFilterStack() : hideFilterStack()
    }

    func showFilterStack() {

        stackFilter.isHidden = false

        UIView.animate(withDuration: 0.3) {
            self.stackFilter.alpha = 1
        }
    }

    func hideFilterStack(animated: Bool = true) {
...
extension ActualVisitVC: BackSelectDate {
    ...
    func selectDate(date: String) {
        ...
        case .dateFrom:
            dateFromLabel.text = date

        case .dateTo:
            dateToLabel.text = date
