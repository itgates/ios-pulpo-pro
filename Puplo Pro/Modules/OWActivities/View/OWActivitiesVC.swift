...
private extension OWActivitiesVC {
    ...
    func validateOfficeWork() -> Bool {
        ...
        guard !ow_type_id.isEmpty else {
            showAlert(alertTitle: "Error", alertMessage: "Select office work")
            return false
        }
        
        guard let comment = commentTextField.text, !comment.isEmpty else {
            showAlert(alertTitle: "Error", alertMessage: "Please enter the comment")
            return false
        }
...
private extension OWActivitiesVC {
    ...
    func bindTableView() {
        ...
        tableViewAM.rx.modelSelected(IdNameModel.self)
        ...
        tableViewOfficeWork.rx.modelSelected(IdNameModel.self)
            .subscribe(onNext: { [weak self] item in
                guard let self else { return }
                self.officeWorkLabel.text = item.name
                self.ow_type_id = item.id ?? ""
