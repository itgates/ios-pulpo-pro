...
final class SlidesWebViewVC: BaseView {
    ...
    private func updateRateButtonAppearance() {
        ...
        var targetIndex = 0
        if let pid = presentationID, let idx = presentations.firstIndex(where: { $0.presentation_id == pid }) {
            targetIndex = idx
        }
...
private extension SlidesWebViewVC {
    func saveSlidesIntoProducts() {
        ...
        if let pid = presentationID {
            if let idx = presentations.firstIndex(where: { $0.presentation_id == pid }) {
        ...
        LocalStorageManager.shared.saveProductsData(model: list)
    }
}
...
private extension SlidesWebViewVC {
    ...
    func saveRating(stars: Int) {
        ...
        guard var presentations = list[productIndex].presentations, !presentations.isEmpty else { return }
        var targetIndex = 0
        if let pid = presentationID, let idx = presentations.firstIndex(where: { $0.presentation_id == pid }) { targetIndex = idx }
